...
class SortWindowsTest {
    ...
    @BeforeAll static void bootstrap() throws Exception {
        ...
        Bootstrap.bootStrap();
        // These headless fixtures need only the stack capacity component normally bound by world loading.
        for (Item item : List.of(Items.DIRT, Items.SAND, Items.GRAVEL, Items.GOLD_INGOT, Items.IRON_INGOT,
            Items.BONE, Items.ROTTEN_FLESH, Items.DIAMOND)) {
    ...
    @Test void fullExactMatchesContinueToTagsThenFallback() throws Exception {
        TagKey<Item> tag = TagKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath("sort_test", "metals"));
    ...
    @Test void exactBeatsTagsRegardlessOfOpeningOrder() throws Exception {
    ...
    @Test void differentComponentsDoNotMerge() throws Exception {
        TestMenu source = new TestMenu(1).put(0, Items.DIRT, 5);
        source.slots.get(0).getItem().set(net.minecraft.core.component.DataComponents.CUSTOM_NAME,
        ...
        assertNotNull(target.slots.get(1).getItem().get(net.minecraft.core.component.DataComponents.CUSTOM_NAME));
    }
    ...
    @Test void duplicateNetworkDestinationsAreRejected() {
        ...
        assertThrows(IllegalArgumentException.class, () -> new DesktopSortWindowsPayload(1, 1, 1, source,
            List.of(target, target), -1, true));
        assertThrows(IllegalArgumentException.class, () -> new DesktopSortWindowsPayload(1, 1, 1, source, List.of(), -1, true));
    }
    ...
    private void assertDragPlacement(int type, int expected) {
        ...
        int preview = AbstractContainerMenu.getQuickCraftPlaceCount(3, type, carried);
